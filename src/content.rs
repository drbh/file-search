...
#[inline]
fn resolve_content_workers_unbounded(requested: usize) -> usize {
    let auto = std::thread::available_parallelism()
        .map(|n| n.get())
        .unwrap_or(8);
    let base = if requested == 0 { auto } else { requested };
...
pub fn query_content_index(
    ...
    if !paths.snapshot.exists() {
        return Err(io_other(
            "content index snapshot not found; run --content-index-build or use --content-search-live",
        ));
    }
...
pub fn query_content_live(
    ...
    let started = Instant::now();
    let workers = resolve_content_workers_unbounded(requested_workers);
    ...
    for handle in handles {
        let output = handle
            .join()
            .map_err(|_| io_other("content live search worker thread panicked"))?;
